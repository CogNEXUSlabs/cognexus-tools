 ...
 *
 * On the first gated tool call the plugin POSTs identity (announce shape,
 * names only) to `POST /api/v1/registry/enroll` with the same Decision API
 * key the gate already holds. The default reply names the adapter to
 * install and never returns a `cnxe_`. A one-time `enrollToken` from
 * Govern / the snippet pack is the only path that returns the envelope
 * key.
 *
...
/** Fire one enroll. Never throws — the caller is the Decision gate. */
export async function enrollInstance(
  ...
  const body: Record<string, unknown> = { agents, skills };
