...
#   agent-governance-python/agent-compliance/src/agent_compliance/prompt_defense.py
...
#
# Upstream changes can be merged manually; do not replace with a PyPI dependency
# unless the team explicitly chooses to.
#
# LOCAL DIVERGENCE — do not lose these in a merge:
#   Three CogNEXUS-only vectors were added after the PocketOS / Cursor / Claude
...
#   carried: `encoding-injection`, `cross-agent-auth`, `least-agency`,
#   `skill-provenance`, `transaction-guardrails`. VECTOR_COUNT is 20.
#
#   Any future vector — ported or local — must be covered by
#   `_COGNEXUS_SYSTEM_DEFENSE_APPENDIX` in security/cognexus_prompt_defense.py
#   **and** by `_DEFENSE_APPENDIX` in the SDK's _helpers.py, in the same
#   change. Both are tuned to grade A against the full rule set, and
#   check_security_sync.py does NOT mirror them — it syncs this vendored file
#   only, so the two appendices drift silently if you forget.
#   `test_augmented_prompt_grades_A` (engine) and the SDK's rule-set grade
#   tests are the canaries.
#
#   Still NOT adopted: upstream's embedding-signal backend (EvidenceSignal /
#   DetectionEvidenceBackend / EmbeddingSignalBackend, in prompt_injection.py)
#   — it puts a model on the decision hot path, which the sub-200 ms P95 and
#   the zero-egress VPC claim both constrain.
...
_RULES: tuple[_DefenseRule, ...] = (
    ...
    # ── Adopted from upstream AGT (agent-governance-toolkit) ──────────
    # Five vectors added upstream after the original vendoring. Ported
    # verbatim; the CogNEXUS-only vectors above are preserved. Adopting
    # these required extending _COGNEXUS_SYSTEM_DEFENSE_APPENDIX in the
    # same change — see security/cognexus_prompt_defense.py — or the
    # engine's own default prompt would drop below grade A.
    _DefenseRule(
