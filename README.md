...
| `kind` | `"tool_call"`: structured-call checks plus the destructive-action guard |

Only `allow` runs the tool; `review` means a human decides first. `decide()`
raises `DecisionError` when the engine refuses to decide (HTTP 503) or cannot
be reached. Treat that as `deny`.
...
for tc in msg.tool_calls or []:
    ...
    try:
        d = artzain.decide(
            action=name,
            target=str(args.get("id") or args.get("to") or "unknown"),
...
for block in (b for b in resp.content if b.type == "tool_use"):
    try:
        d = artzain.decide(
            action=block.name,
            target=str(block.input.get("id") or block.input.get("to") or "unknown"),
            payload=json.dumps({"tool": block.name, "arguments": block.input}),
            kind="tool_call",
        )
    except artzain.DecisionError:
        d = {"outcome": "deny"}  # the engine did not decide: fail closed
...
and the line escalates every `medium` vote, not just this one. Contracts are
checked only for `kind="tool_call"`, on a running engine with the bundle
active (`artzain local up` is enough). Offline, with no API key, `decide()`
runs the local guards only: `offline: true`, nothing sealed, no bundle.
